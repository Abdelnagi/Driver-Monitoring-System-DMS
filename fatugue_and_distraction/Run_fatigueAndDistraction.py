...
from yawnBlinkDetection import DrowsinessDetector
from gazeHeadDetection import GazeAndHeadDetection  # Import your class
import os
import json
from flask import Flask, Response

import yawnBlinkDetection as yb        # Import the script to access its global variables
importlib.reload(yb)                   # ✅ This forces Python to reload the latest changes

import gazeHeadDetection as gh         # Import the script to access its global variables
...
class DriverMonitorApp:
    def __init__(self):
        ...
        self.abnormal_head_start_time = None
    ...
    def update_info(self):
        ...
        # Track abnormal head movement duration ############### NEW #######################
        abnormal_states = ["ABNORMAL PITCH", "ABNORMAL YAW", "ABNORMAL ROLL"]
        if up_head_status_gui in abnormal_states:
            if self.abnormal_head_start_time is None:
                self.abnormal_head_start_time = time.time()
            elif time.time() - self.abnormal_head_start_time >= abnormal_head_sleep_threshold:
                self.sleep_alert = "on"
        else:
            self.abnormal_head_start_time = None
    ...
    def update_status_file_camera2(self, alert_text=""):
        ...
        status_camera2 = {
            ...
            "pitch": f" {gh.pitch_gui:.2f} deg",
            "yaw": f" {gh.yaw_gui:.2f} deg",
            "roll": f" {gh.roll_gui:.2f} deg",
            "head_status": gh.head_status_gui,
            "distraction": f"within 3 min: {gh.distraction_counter}",
            "blinks": f" {yb.num_of_blinks_gui}",
            "microsleep_duration": f" {yb.microsleep_duration_gui:.2f} sec",
            "yawns": f" {yb.num_of_yawns_gui}",
            "yawn_duration": f" {yb.yawn_duration_gui:.2f} sec",
            "blinks_per_minute": f" {yb.blinks_per_minute_gui}",
            "yawns_per_minute": f" {yb.yawns_per_minute_gui}",
            "alert": alert_text
        }
