...
# Global and helper variables for GUI updates
pitch_gui = 0.0
yaw_gui = 0.0
roll_gui = 0.0
gaze_gui = "Center"
...
class GazeAndHeadDetection:
    ...
    baseline_threshold_time = 20  # Time in seconds to collect baseline data
        ...
        if results.multi_face_landmarks:  # If a face is detected
            for face_landmarks in results.multi_face_landmarks:
                ...
                if not GazeAndHeadDetection.baseline_set:
                    ...
                    if elapsed_time >= GazeAndHeadDetection.baseline_threshold_time:
    ...
    def process_yawn_blink(self):
        ...
        while self.running:
            ...
            # Store the latest prediction and confidence
            self.latest_prediction, self.latest_confidence = self.yawn_blink_detector.eyes_state, getattr(self.yawn_blink_detector, 'confidence', 0.0)
            self.eyes_state, self.confidence = self.predict()

    def show_frame(self):
        if self.frame is not None:
            # Copy the frame to avoid drawing on the original
            display_frame = self.frame.copy()
            # Get the latest prediction if available
            prediction = getattr(self, 'latest_prediction', '')
            confidence = getattr(self, 'latest_confidence', 0.0)
            text = f"{prediction} ({confidence:.2f})" if prediction else ""
            # Draw the text on the frame
            cv2.putText(display_frame, text, (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
